...
public:
  ...
  void write_log(AllocInfo &info) {
    ...
    format_as_csv_entry(log_line_buf_, "alloc", info.bytes, info.align, info.retval, info.processing_time);
    write_log_line();
  }
  ...
  void write_log(GetBlockSizeInfo &info) {
    format_as_csv_entry(log_line_buf_, "get_block_size", info.ptr, info.retval, info.processing_time);
    write_log_line();
  }

  void write_log(AllocZeroedInfo &info) {
    format_as_csv_entry(log_line_buf_, "alloc_zeroed", info.bytes, info.retval, info.processing_time);
    write_log_line();
  }

  void write_log(ReallocInfo &info) {
    format_as_csv_entry(log_line_buf_, "realloc", info.ptr, info.new_size, info.retval, info.processing_time);
    write_log_line();
  }
